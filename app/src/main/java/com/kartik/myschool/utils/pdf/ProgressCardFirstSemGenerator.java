...
public class ProgressCardFirstSemGenerator {
    ...
    private static void buildRightPanel(PdfPCell panel, Context ctx, ClassModel cls, MarksRecord rec, MarksRecord bestRemarkRec) throws Exception {
        ...
        for (int i = 0; i < 9; i++) {
            String sName = i < subjects.size() ? subjects.get(i).name : (i < defaultSubs.length ? defaultSubs[i] : "");
            String grade = "-";
            if (rec != null && sName != null && !sName.isEmpty()) {
                MarksRecord.SubjectMarksDetail d = PdfGenerator.detail(rec, sName);
