...
public class GunapattrakGenerator {
    ...
    public static void addGunapattrakContent(Document doc, Context ctx, School school, ClassModel cls, Student student,
        ...
        String termLabel = (rec == sem2) ? PdfLocalizer.get(ctx, "द्वितीय सत्र", "Second Semester")
        ...
        for (int i = 0; i < (subjects != null ? subjects.size() : 0); i++) {
            ...
            if (d != null) {
                ...
                if (isNonAcademic) {
                    cellSpan(tbl, " ", fSmall, bg, C_DARK, 4, 2, Element.ALIGN_CENTER); // Merge summative columns and
                                                                                        // both rows
                } else {
